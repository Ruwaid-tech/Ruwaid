...
class InventoryFrame(BaseFrame):
    def __init__(self, parent: tk.Widget, app: KitchenManagerApp) -> None:
        ...
        self.tree = ttk.Treeview(
            wrapper,
            columns=("name", "quantity", "unit", "threshold", "status"),
            show="headings",
            selectmode="browse",
        )
        self.tree.grid(row=1, column=0, sticky="nsew", pady=20)
        for col, heading, anchor, width in (
            ("name", "Ingredient", tk.W, 220),
            ("quantity", "Quantity", tk.CENTER, 120),
            ("unit", "Unit", tk.CENTER, 120),
            ("threshold", "Low stock at", tk.CENTER, 140),
            ("status", "Status", tk.CENTER, 100),
        ):
