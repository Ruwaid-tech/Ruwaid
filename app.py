...
class DashboardFrame(BaseFrame):
    def __init__(self, parent: tk.Widget, app: KitchenManagerApp) -> None:
        ...
        ttk.Label(header, textvariable=self.title_var, font=("Segoe UI", 24, "bold")).grid(
            row=0, column=0, sticky="w"
        )
        self.user_var = tk.StringVar(value="User:(guest)")
        ttk.Label(header, textvariable=self.user_var, font=("Segoe UI", 14)).grid(row=0, column=1, sticky="e")
        ...
        ttk.Label(footer, textvariable=self.status_var, font=("Segoe UI", 12)).pack(side=tk.LEFT)
...
class InventoryEditFrame(BaseFrame):
    def __init__(self, parent: tk.Widget, app: KitchenManagerApp) -> None:
        ...
        ttk.Label(header, text="Edit inventory", font=("Segoe UI", 22, "bold")).pack(side=tk.LEFT)
        nav_bar = ttk.Frame(header)
        nav_bar.pack(side=tk.RIGHT)
        self.add_nav_buttons(nav_bar)
        ...
        ttk.Label(form, textvariable=self.item_name_var, font=("Segoe UI", 11, "bold")).grid(
...
class SuppliersFrame(BaseFrame):
    def __init__(self, parent: tk.Widget, app: KitchenManagerApp) -> None:
        ...
        header = ttk.Frame(wrapper)
        header.grid(row=0, column=0, columnspan=2, sticky="ew")
        ttk.Label(header, text="Suppliers", font=("Segoe UI", 22, "bold")).pack(side=tk.LEFT)
...
class VendorOrdersFrame(BaseFrame):
    ...
    def __init__(self, parent: tk.Widget, app: KitchenManagerApp) -> None:
        ...
        header = ttk.Frame(wrapper)
        header.grid(row=0, column=0, sticky="ew")
        ttk.Label(header, text="Vendor orders", font=("Segoe UI", 22, "bold")).pack(side=tk.LEFT)
        nav = ttk.Frame(header)
        nav.pack(side=tk.RIGHT)
        self.add_nav_buttons(nav)
...
class CustomerOrdersFrame(BaseFrame):
    ...
    def __init__(self, parent: tk.Widget, app: KitchenManagerApp) -> None:
        ...
        header = ttk.Frame(wrapper)
        header.grid(row=0, column=0, sticky="ew")
        ttk.Label(header, text="Customer orders", font=("Segoe UI", 22, "bold")).pack(side=tk.LEFT)
        nav = ttk.Frame(header)
        nav.pack(side=tk.RIGHT)
        self.add_nav_buttons(nav)
...
class SettingsFrame(BaseFrame):
    ...
    def __init__(self, parent: tk.Widget, app: KitchenManagerApp) -> None:
        ...
        header = ttk.Frame(wrapper)
        header.grid(row=0, column=0, sticky="ew")
        ttk.Label(header, text="Settings", font=("Segoe UI", 22, "bold")).pack(side=tk.LEFT)
        nav = ttk.Frame(header)
        nav.pack(side=tk.RIGHT)
        self.add_nav_buttons(nav)
        ...
        for idx, (key, label) in enumerate(self.LABELS.items()):
            ...
            var = tk.StringVar()
            ttk.Entry(form, textvariable=var).grid(row=idx, column=1, sticky="ew", pady=6)
    ...
    def load_settings(self) -> None:
        ...
        for key, var in self.setting_vars.items():
            var.set(stored.get(key, self.DEFAULTS.get(key, "")))

    def save_settings(self) -> None:
        for key, var in self.setting_vars.items():
            self.app.db.set_setting(key, var.get().strip())
