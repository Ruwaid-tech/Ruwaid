...
- `main.py` – Thin entry point that boots the PyQt application.
- `init_db.py` – Helper to initialize/seed the SQLite database without launching the UI.
...
- `images/` – Placeholder directory for artwork thumbnails if you add them.
...
4. **Admin flow**
   - Log in as the admin to view new-order notifications, mark them contacted, review full order history, manage artworks via CRUD, and edit owner contact info.
